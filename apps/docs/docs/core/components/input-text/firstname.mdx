...
This example uses the `FormikFormBuilder` to render a single-line text input for collecting a user’s first name. The field is validated using **Yup** and defined in the `firstNameField` config variable. Check [Validation Rules](/core-docs/configuration#validation-rules) for more detail.
