...
This example uses the `FormikFormBuilder` to render a username input field for collecting a user’s username. The field is validated using **Yup** and defined in the `usernameField` config variable. Check [Validation Rules](/docs/getting-started/my-doc-configuration#validation-rules) for more detail.
...
import { FormBuilder, FieldType, InputTypes } from "@mjfy/core";
