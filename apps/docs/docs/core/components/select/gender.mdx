...
This example uses the `FormikFormBuilder` to render a gender select field for selecting a user’s gender. The field is validated using **Yup** and defined in the `genderField` config variable. Check [Validation Rules](/core-docs/configuration#validation-rules) for more detail.
