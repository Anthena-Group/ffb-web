...
This example uses the `FormikFormBuilder` to render a role select field for selecting a user’s role. The field is validated using **Yup** and defined in the `roleField` config variable. Check [Validation Rules](/core-docs/configuration#validation-rules) for more detail.
