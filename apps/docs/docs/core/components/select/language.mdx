...
This example uses the `FormikFormBuilder` to render a language select field for selecting a user’s preferred language. The field is validated using **Yup** and defined in the `languageField` config variable. Check [Validation Rules](/core-docs/configuration#validation-rules) for more detail.
