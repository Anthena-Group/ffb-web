...
This example uses the `FormikFormBuilder` to render([FormikRenderer](/core-docs/configuration#formikrenderer)).
The field is validated using Yup and defined in confirmAction config variable. Check [Validation Rules](/core-docs/configuration#validation-rules) for more detail.
